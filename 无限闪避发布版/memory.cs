    ...
    public abstract class Helper
    {
        ...
        public static int ReadMemoryValue32(int baseAddress, string processName)
        {
            try
            {
                ...
                ReadProcessMemory(hProcess, (IntPtr)baseAddress, byteAddress, sizeof(int), IntPtr.Zero); //将制定内存中的值读入缓冲区
        ...
        public static float ReadMemoryFloat16(int baseAddress, string processName)
        {
            try
            {
                byte[] buffer = new byte[4];
        ...
        public static short ReadMemoryValue16(int baseAddress, string processName)
        {
            try
            {
                ...
                IntPtr byteAddress = Marshal.UnsafeAddrOfPinnedArrayElement(buffer, 0); //获取缓冲区地址
                IntPtr hProcess = OpenProcess(0x1F0FFF, false, GetPidByProcessName(processName));
                ReadProcessMemory(hProcess, (IntPtr)baseAddress, byteAddress,sizeof(short), IntPtr.Zero); //将制定内存中的值读入缓冲区
                CloseHandle(hProcess);
        ...
        public static void WriteMemoryValue32(int baseAddress, string processName, int value)
        {
            ...
            WriteProcessMemory(hProcess, (IntPtr)baseAddress, new int[] { value }, sizeof(int), IntPtr.Zero);
            CloseHandle(hProcess);
        }
        public static void WriteMemoryValue32(int baseAddress, string processName, float value)
        {
            byte []bd=BitConverter.GetBytes(value);
            IntPtr hProcess = OpenProcess(0x1F0FFF, false, GetPidByProcessName(processName)); //0x1F0FFF 最高权限
            IntPtr writevalue=new IntPtr();
            bool a = WriteProcessMemory(hProcess, (IntPtr)baseAddress, bd, sizeof(float), writevalue);
            int nums = (int)writevalue;
            Console.WriteLine(string.Format("{0}=>{1}",value,nums));
            CloseHandle(hProcess);
        }
        public static void WriteMemoryValue16(int baseAddress, string processName,short value)
        {
            IntPtr hProcess = OpenProcess(0x1F0FFF, false, GetPidByProcessName(processName)); //0x1F0FFF 最高权限
            WriteProcessMemory(hProcess, (IntPtr)baseAddress, new int[] { value }, sizeof(short), IntPtr.Zero);
            CloseHandle(hProcess);
        }
    }
}
